...
class InterAssistQuoteProvider:
    ...
    def _daily_price(
        ...
        if trip_days <= 90:
            bracket = next(b for b in _DAILY_BRACKETS if trip_days <= b)
            key = f"_{bracket}_dias{age_suffix}"
            return self._parse_decimal(raw.get(key))

        base = self._parse_decimal(raw.get(f"_90_dias{age_suffix}"))
        extra_day = self._parse_decimal(raw.get(f"dia_adicional{age_suffix}"))
        if base is None or extra_day is None:
            return None
        extra_days = trip_days - 90
        return base + (extra_day * Decimal(extra_days))

    def _annual_price(
        self, raw: dict[str, Any], trip_days: int, age_suffix: str
    ) -> Decimal | None:
        # Multiviaje: tramo anual por días máximos corridos (30/60/90).
        if trip_days > 90:
            return None
        bracket = next(b for b in _ANNUAL_BRACKETS if trip_days <= b)
        if age_suffix:
